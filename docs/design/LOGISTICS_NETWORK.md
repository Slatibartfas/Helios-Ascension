...
| 1 | Construction panel shows "Waiting for freighter" / "Awaiting resources" | ✅ Shipped (v0.4.x) | `src/ui/construction/queue.rs` + `src/ui/construction/tooltip.rs` (the bevy_ui path; the legacy `src/ui/construction_panel.rs` was deleted in v0.5.2 along with the canary-era `src/ui/construction.rs` and refactored into the `src/ui/construction/` directory) |
...
- [x] UI in construction panel: per-resource minimum input fields (`render_minimum_stockpile_editor` in `src/ui/construction/state.rs` / `src/ui/construction/data.rs` (line numbers shifted; the legacy `src/ui/construction_panel.rs:2473` is now `src/ui/construction/state.rs` after the v0.5.2 split into the `src/ui/construction/` directory))
...
- `src/ui/construction/queue.rs` + `src/ui/construction/tooltip.rs` (the bevy_ui path; the legacy `src/ui/construction_panel.rs` was deleted in v0.5.2 along with the canary-era `src/ui/construction.rs` and refactored into the `src/ui/construction/` directory) — "Awaiting resources" / "Waiting for freighter" badges
