...
fn setup_solar_system(
    ...
    for body_data in &data.bodies {
        ...
        let rotation_speed = if body_data.rotation_period != 0.0 {
            (2.0 * std::f32::consts::PI) / (body_data.rotation_period.abs() * 86400.0)
                * if body_data.rotation_period < 0.0 { -1.0 } else { 1.0 }
        } else {
            0.0
        };
    ...
    for body_data in &data.bodies {
        if let Some(ref orbit) = body_data.orbit {
            ...
            // Get parent entity
            let parent_entity = body_data.parent.as_ref()
...
fn update_orbits(
    ...
    for (mut transform, mut orbit, _body) in query.iter_mut() {
        ...
        // Apply inclination
        let y = orbit.semi_major_axis * orbit.inclination.to_radians().sin() * orbit.current_angle.sin();
