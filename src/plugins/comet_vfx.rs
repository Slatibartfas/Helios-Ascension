...
fn setup_comet_effects(
    ...
    // removed local variable def here
    ...
    // Add property to module via writer
    let solar_wind_prop = writer.add_property("solar_wind_force", (Vec3::Y * 10.0).into());
    ...
    let radiation_pressure_prop = writer_dust.add_property("radiation_pressure", (Vec3::Y * 2.0).into());
    ...
    // Init: Velocity = Comet Velocity * 0.9
    // Need to access property 'comet_velocity' in Init
    let init_vel_dust = SetAttributeModifier::new(
        Attribute::VELOCITY,
        writer_dust.prop(comet_vel_prop).mul(writer_dust.lit(0.9)).expr(),
    );
        ...
        for &child in children.iter() {
            if let Ok((mut properties, name)) = effect_query.get_mut(child) {
                if name.as_str() == "Ion Tail" {
                    // Ion tail needs Solar Wind Vector
