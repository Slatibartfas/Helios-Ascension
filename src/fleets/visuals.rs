    ...
    for (entity, maneuver, maybe_orbit) in fleet_query.iter() {
        ...
        if center_is_star && *view_mode == ViewMode::System {
            ...
            let (dest_visual_r, dest_ring_r) = body_query
                ...
                .unwrap_or((0.0, 0.0));
            let is_inter_star =
                is_inter_star_transfer(maneuver.origin_body, maneuver.destination_body, &body_query);
...
pub fn draw_fleet_transfer_preview(
    ...
    let is_inter_star = is_inter_star_transfer(origin_body, target_entity, &body_query);
    ...
    let (dp, cv_ref) = if is_inter_star {
