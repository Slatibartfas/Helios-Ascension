...
impl PorkchopConfig {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if self.defaults.resolution_t_dep * self.defaults.resolution_tof > 20000 {
            violations.push(format!(
                "defaults.resolution_t_dep * defaults.resolution_tof must be ≤ 20000 (got {} × {} = {})",
        ...
        for (i, ov) in self.category_overrides.iter().enumerate() {
            if ov.resolution_t_dep * ov.resolution_tof > 20000 {
                violations.push(format!(
                    "category_overrides[{}] ({}): resolution product {} × {} = {} > 20000",
                    i,
                    ov.match_key,
                    ov.resolution_t_dep,
                    ov.resolution_tof,
...
mod tests {
    ...
    #[test]
    fn porkchop_config_resolution_over_20000_fails_validation() {
        let mut cfg = PorkchopConfig::default();
        cfg.defaults.resolution_t_dep = 150;
        cfg.defaults.resolution_tof = 150;
        let v = cfg.validate().unwrap_err();
        assert!(v.iter().any(|s| s.contains("20000")));
    }
