...
impl EventBus {
    ...
    pub fn publish(&self, event: GameEvent) {
        for (i, sub) in self.subscriptions.iter().enumerate() {
            ...
            let matches = matches!((&sub.filter_category, &event), (None, _) | (Some(EventCategory::Alert), GameEvent::Alert { .. }) | (Some(EventCategory::Random), GameEvent::Random { .. }) | (Some(EventCategory::Story), GameEvent::Story { .. }));
            if matches {
