...
impl Colony {
    ...
    ///
    /// Per-capita food consumption: 0.0000011 Mt/person/yr (1,100 kg/person/yr,
    /// FAO 2024 SOFA world food supply). At this scale 1 Farm (360 Mt/yr) feeds
    /// ~327M people, and 25 Farms feed 8.2B Earth — within the 10–50
    /// manageable-building constraint (target ~25 Farms per body).
    ///
    /// **NOTE**: this is a *hard-coded* fallback. The RON `FoodProduction`
    /// modifier in `buildings.ron` is documentation, not read by this
    /// function. If the calibration is changed, both the values here AND
    /// the RON modifier values must be updated in lockstep. See
    /// `docs/design/BALANCE_PATCHES_v0.5.md` §4.1 + §8.1 + §8.3.1–§8.3.4.
    ...
    /// FAO 2024 SOFA world food supply). At this scale 25 Farms × 360 Mt/yr =
    /// 9,000 Mt/yr ≈ 8.2B × 0.0000011 = 9,020 Mt/yr (balanced).
    ///
    /// Unit conversion note: 1,100 kg = 1.1 × 10⁻⁶ Mt (since 1 Mt = 10⁹ kg).
    /// The previous canary value 0.0011 was 1,000× too large (0.0011 Mt =
    /// 1,100 tonnes, not 1,100 kg). v0.5.1 corrects this and the
    /// hard-coded per-build values in `food_production_per_year` to
    /// restore internal balance (9,020 Mt/yr demand ≈ 9,000 Mt/yr supply).
    ///
    /// Calibration: docs/design/BALANCE_PATCHES_v0.5.md §4.1 + §8.1.
...
mod tests {
    ...
    fn test_operating_cost() {
        ...
        colony.add_building(BuildingType::IronMine);
        assert!((colony.operating_cost_per_year() - 75.0).abs() < 0.001);
    }
    ...
    fn test_earth_yield_one_unchanged() {
        ...
        assert!(
            (earth.food_production_per_year() - 360.0).abs() < 0.001,
