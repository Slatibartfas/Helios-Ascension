//! Survey events fired by the mission lifecycle system.
//!
//! PR-B (GRA-80) wires up the mission state machine and fires a
//! [`SurveyEvent`] for every state transition. The dossier UI and
//! the notification surface subscribe via `MessageReader<SurveyEvent>`.
//!
//! PR-C's three anomaly variants (`AnomalyDetected`, `AnomalyActivated`,
//! `AnomalyRefuted`) live alongside PR-B's mission variants in the
//! same flat enum. Consumers pattern-match on the variant.
//!
//! Dispatch and abort are modelled as separate Bevy events so the
//! UI can enqueue them without touching the mission's storage
//! directly — see the [action-queue decoupling] rule.
//!
//! [action-queue decoupling]: ../../../AGENTS.md
...
///
/// The `body` field is the celestial body the mission is targeting;
/// `mission_id` matches [`ActiveSurveyMission::id`](
/// crate::survey::components::ActiveSurveyMission::id).
///
...
pub enum SurveyEvent {
    // ── PR-C: anomaly confidence model events ───────────────────────
