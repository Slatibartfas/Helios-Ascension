...
/// ...)`.
///
/// `#[allow(dead_code)]` keeps this helper compile-clean on
/// `cargo clippy --all-targets -- -D warnings` while it's
/// referenced only by the doc comment above.  The function is
/// documentation of the math the test fixtures assume; the
/// in-test invocations of the same formula inline it.
#[allow(dead_code)]
