...
fn best_buildable_with_no_research_picks_light_freighter() {
    // No-research baseline per `docs/design/SHIP_TEMPLATES.md` "Worked
    // example": with no research, the AI picks `light_freighter`.
    // `light_freighter.required_tech: None` in
    // `freighter_templates.ron` makes this the fallthrough.
    let app = build_app_with_templates_loaded();
    let registry = registry(&app);
    let data = data(&app);
    let entry = registry
