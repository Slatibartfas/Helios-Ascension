...
Background music is generated by **MiniMax Music 3.0** as ambient cinematic tracks tailored for hard-sci-fi 4X gameplay. Each track is prompted with a specific scenario (exploration, fleet maneuvers, mining, late-game reflection, etc.) and the exact prompt and parameters are logged in `assets/data/music_prompts.ron` for reproducibility.

The current playlist (`src/plugins/music.rs::MusicPlaylist::default()`) loops ten AI-generated tracks; the bottom-right of the time bar shows the currently-playing title with play/pause, skip, and volume controls.

Music files are stored in `assets/audio/music/`. To add more tracks, push a new `TrackInfo` entry into `MusicPlaylist::default()` and append the matching prompt to `assets/data/music_prompts.ron`.
